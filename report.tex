...
\begin{equation}
V_t + \sigma^2 S_i^2 V_{\overline{x}x} + r S_i V_x + rV_i = 0
\end{equation}
...
\end{alignat}
The latter is a so-called ``normal equation''. It is very convinient to use allowing to obtain solution in one step. This contrasts to iterative methods of solving optimization problem, where you would need to normalize independent variables, calculate gradient of cost function $J(\theta)$ and searching for optimal step size. 
