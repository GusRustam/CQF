...
%Cap is a set of caplets, and each of them is an option on future interest rate of given term. Hence, price of a cap is an expected price of sum of caplet prices. In case that there are $M$ simulations and each cap consists of $K$ caplets, its price is:
%\begin{equation}
%C = \frac{1}{M}\sum_{k=1}^{M}\left(\sum_{i=0}^K \text{Price}_{\text{Caplet, i}}\right)
%\end{equation}
%Price of a caplet is equal to its discounted payoff. Let's suppose that all caplets have same strike rate R, and $i$th caplet has term $t_i$ then caplet price is:
%\begin{equation}
%\text{Price}_{\text{Caplet,i}} = \frac{(r_{t_i} - R)^+}{1+r_{t_i}}
%\end{equation}
